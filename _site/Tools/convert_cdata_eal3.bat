...
SET "JMS_Description=Convert EA Layer 3 CDATA to MP3"
...
SET "APPNAME=CONVERT CDATA [%JMS_Name% - %JMS_Version%]"
...
:: 
:: Usage: ealayer3.exe InputFilename [Options]
:: 
...
:: 
:: Encoding: ealayer3.exe -E InputFile [InputFile2 ...] [Options]
...
SET "CDATA=%SDK%\cdata.txt"
SET "CDATA_KW=%SDK%\cdata_kw.txt"
SET "EAL3=%SDK%\ealayer3.exe"
...
SET OPTS=-m --no-banner

REM "%EAL3%" "input<cdata>" %OPTS% -o "output<mp3>"

GOTO KW

:TW
FOR /F "usebackq tokens=* delims=" %%G IN ("%CDATA%") DO (
	...
	ECHO. >%DBGMAC%
	ECHO Converting !FILEN!!FILEX! to !FILEN!.mp3 . . . >%DBGMAC%
	ECHO. >%DBGMAC%

	"%EAL3%" "!FILE!" !OPTS! -o "!FILED!!FILEN!.mp3"
...
:KW
FOR /F "usebackq tokens=* delims=" %%G IN ("%CDATA_KW%") DO (
	REM CALL Our duration start subroutine
	CALL :DURATION_START

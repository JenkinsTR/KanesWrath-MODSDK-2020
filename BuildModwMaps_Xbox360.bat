...
if exist "%XBBIGOUT%\%modname%_%modversion%.cfg" del "%XBBIGOUT%\%modname%_%modversion%.cfg" /F /Q
if exist "%XBBIGOUT%\%modname%_%modversion%_Streams.big" echo add-big %modname%_%modversion%_Streams.big>> "%XBBIGOUT%\%modname%_%modversion%.cfg"
if exist "%XBBIGOUT%\%modname%_%modversion%_Misc.big" echo add-big %modname%_%modversion%_Misc.big>> "%XBBIGOUT%\%modname%_%modversion%.cfg"
...
if exist "%cd%\Xbox360\Mods\%modname%\LanguagePacks" (
	...
	for /D %%A in ("*") do (
		...
		::MakeBig
		if exist "!sdk!\Compilation\Xbox360\Mods\%modname%\LanguagePacks\%%A" "!sdk!\Tools\MakeBig.exe" -f "!sdk!\Compilation\Xbox360\Mods\%modname%\LanguagePacks\%%A" -o:"%XBBIGOUT%\%modname%_%modversion%_Streams_%%A.big"
		if exist "!cd!\%%A\Misc" "!sdk!\Tools\MakeBig.exe" -f "!cd!\%%A\Misc" -o:"!sdk!\Compilation\Xbox360\Mods\%modname%\LanguagePacks\%%A" -o:"%XBBIGOUT%\%modname%_%modversion%_Misc_%%A.big"
	)
...
FOR /R %%G IN ("%cd%\Xbox360\Mods\%modname%\Data\Maps\*.map") DO (
	SET mapname=%%~nG

	REM :mapname -- not needed as we're pulling the mapname from the .map filename
	REM if not defined mapname set /P mapname="Map Name: "
	REM if not defined mapname goto mapname
	
	:: Begin compile
	echo.
	echo !mapname!
	
	:: Check for map data -- we don't have to since we're in a FOR loop
	REM if not exist "%cd%\Xbox360\Mods\%modname%\Maps\!mapname!" (
	...
	echo Compiling Map...
	if exist "%cd%\Xbox360\Mods\%modname%\Maps\!mapname!\map.xml" "%WEMMG%" "%cd%\Xbox360\Mods\%modname%\Maps\!mapname!\map.xml"
	if exist "%cd%\Xbox360\Mods\%modname%\Maps\!mapname!\mapmetadata_!mapname!.xml" "%wrathed%" -gameDefinition:"Kane's Wrath" -compile:"%cd%\Xbox360\Mods\%modname%\Maps\!mapname!\mapmetadata_!mapname!.xml" -art:"Art" -audio:"Audio" -out:"%cd%\Compilation\Xbox360\Maps\!mapname!\Data\AdditionalMaps" -version:""
	if exist "%cd%\Xbox360\Mods\%modname%\Maps\!mapname!\mapmetadata_!mapname!.xml" del "%cd%\Xbox360\Mods\%modname%\Maps\!mapname!\mapmetadata_!mapname!.xml" /F /Q
	
	if exist "%cd%\Compilation\Xbox360\Maps\!mapname!\Data\AdditionalMaps\MapMetaData_!mapname!.manifest" if exist "%cd%\Xbox360\Mods\%modname%\Maps\!mapname!\Assets\AdditionalMaps\MapMetaData_!mapname!" (
		"%WEAM%" "%cd%\Compilation\Xbox360\Maps\!mapname!\Data\AdditionalMaps\MapMetaData_!mapname!.manifest" "%cd%\Xbox360\Mods\%modname%\Maps\!mapname!\Assets\AdditionalMaps\MapMetaData_!mapname!"
		"%WESS%" "%cd%\Compilation\Xbox360\Maps\!mapname!\Data\AdditionalMaps\MapMetaData_!mapname!.manifest"
	)
	
	if exist "%cd%\Xbox360\Mods\%modname%\Maps\!mapname!\map.xml" "%wrathed%" -gameDefinition:"Kane's Wrath" -compile:"%cd%\Xbox360\Mods\%modname%\Maps\!mapname!\map.xml" -map -terrain:"Game Files\Art\Terrain" -art:"Art" -audio:"Audio" -data:"Data" -out:"%cd%\Compilation\Xbox360\Maps\!mapname!\Data\!mapname!" -version:""
	
	if exist "%cd%\Compilation\Xbox360\Maps\!mapname!\Data\!mapname!\Map.manifest" if exist "%cd%\Xbox360\Mods\%modname%\Maps\!mapname!\Assets\Map" (
		"%WEAM%" "%cd%\Compilation\Xbox360\Maps\!mapname!\Data\!mapname!\Map.manifest" "%cd%\Xbox360\Mods\%modname%\Maps\!mapname!\Assets\Map"
	...
	if exist "%cd%\Xbox360\Mods\%modname%\Maps\!mapname!\Misc" "%WEBIG%" -f "%cd%\Xbox360\Mods\%modname%\Maps\!mapname!\Misc" -o:"%XBBIGOUT%\Map_Misc.big"
	
	:: We need to use filesystem.cfg instead of .skudef for Xbox
	if exist "%cd%\Xbox360\Mods\%modname%\Maps\!mapname!\filesystem_map.cfg" del "%XBBIGOUT%\filesystem_map.cfg" /F /Q
	...
	:: We also need to pool the new filesystem_map files into one cfg (NOTE THE PLURAL)
	if exist "%XBBIGOUT%\filesystem_map.cfg" echo try-add-config Mods\%modname%\Maps\!mapname!\filesystem_map.cfg>> "%cd%\Xbox360\BuiltMods\%modname%\filesystem_maps.cfg"
